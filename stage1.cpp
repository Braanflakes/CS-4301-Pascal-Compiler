...
int keyCount = 12;
string keys[12] = {"program", "begin", "end", "var", "const",
...
bool ADD_LEVEL_OP();
bool MULT_LEVEL_OP();
void Code(string, string operand1 = "", string operand2 = "");
bool isEXPRESS();
...
void BeginEndStmt(){
	...
	if(NonKeyID() || token == "read" || token == "write" || token == ";" || token == "begin"){
		EXEC_STMTS();
	}if(token != "end"){
...
//STAGE 1 NEW PRODUCTIONS EJ FROM //2 TO //18
const string EXPRESSFail = "Invalid expression: \"not\", \"true\", \"false\", \"(\", \"+\", \"-\", non-key ID, or integer expected";

//2
void EXEC_STMTS(){
	if(NonKeyID() || token == "read" || token == "write" || token == "begin" ||  token == ";"){
			EXEC_STMT();
			EXEC_STMTS();
	}else if(token != "end")
...
void Code(string myOperator, string operand1, string operand2){
	if(myOperator == "program"){
		...
		if(operand1 == "" || operand2 == "")
			Error("1addition requires two operands"); //addition must be binary
		...
		if(operand2 != "")
			Error("negation requires one operand");
		EmitNegCode(operand1);
	}else if(myOperator == "not"){
		if(operand2 != "")
			Error("not operation requires one operand");
		EmitNotCode(operand1);
	}else if(myOperator == "*"){
		if(operand1 == "" || operand2 == "")
			Error("2addition requires two operands"); //addition is binary
		EmitMultiplicationCode(operand1,operand2);
	}else if(myOperator == "div"){
		if(operand1 == "" || operand2 == "")
			Error("division requires two operands"); //division is binary
		EmitDivisionCode(operand1,operand2);
	}else if(myOperator == "mod"){
		if(operand1 == "" || operand2 == "")
			Error("mod requires two operands");//modulo is binary
		EmitModuloCode(operand1,operand2);
	}else if(myOperator == "and"){
		if(operand1 == "" || operand2 == "")
			Error("and requires two operands"); //and is binary
		EmitAndCode(operand1,operand2);
	}else if(myOperator == "or"){
		if(operand1 == "" || operand2 == "") //or is binary
		...
		if(operand1 == "" || operand2 == "")
			Error("3addition requires two operands"); //= is binary
		EmitEqualityCode(operand1,operand2);
	}else if(myOperator == ":="){
		if(operand1 == "" || operand2 == "")
			Error("assignment requires two operands"); //:= is binary
		EmitAssignCode(operand1, operand2);
	}else if(myOperator == "<>"){
		if(operand1 == "" || operand2 == "")
			Error("<> requires two operands"); //!= (aka <>) is binary
		EmitNotEqualCode(operand1,operand2);
	}else if(myOperator == ">"){
		if(operand1 == "" || operand2 == "")
			Error("> requires two operands"); //greater than is binary
		EmitGreaterThanCode(operand1,operand2);
	}else if(myOperator == ">="){
		if(operand1 == "" || operand2 == "")
			Error(">= requires two operands"); //>= is binary
...
void EmitNegCode(string operand1)
{
	...
	for(int i = 0; i < symbolTable.size(); ++i)
	{
		...
		{
			operand = i;
			temp == symbolTable[i].internalName;
		}
	}
